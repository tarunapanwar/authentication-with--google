...
import "./globals.css";
import { Header, TextField } from "../app/Components";
import { FaUser } from "react-icons/fa";
import { IOrganization } from "@/Interfaces/workspace";
...
const AddWorkspace = () => {
  ...
  const handleSubmit = async(values: IOrganization) => {
    if(values){
      setLoading(true);
      const response = await axios.post<IApiResponse>('api/workspaces', values);
  ...
  return (
    <>
      <Header />
      <div className="flex flex-col items-center relative top-20 p-10">
        <div>
